...
def main(prefix):
    ...
    z = zarr.open("test.zarr", mode="w", shape=(c.bounds[2], c.bounds[3], 3), chunks=(1024,1024), dtype=np.uint16)
    counter = zarr.open("counter.zarr", mode="w", shape=(int(c.bounds[2]), int(c.bounds[3]), 3), chunks=(1024,1024),dtype=np.uint8)
    ...
    z  = dask.array.from_array(z, chunks=(1024,1024,3))
    counter = dask.array.from_array(counter, chunks=(1024,1024,3))
    z = z//counter
